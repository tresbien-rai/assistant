...
| **CT-06 polish** ✅ | Three fixes from live testing. (1) Tools-off note now re-renders when file tools are flipped while the panel is open (`FilePanel.onToolsToggled`), not only on next open. (2) Toggling a knowledge file back to its container default now **clears** the override instead of writing a redundant one — `containerEnabled` added to the `/context` per-file payload so the panel can tell; keeps "an override exists only when the chat deviates" literally true (no phantom `↺` marker/border). (3) Badge digit vertically centred (`line-height: 0`, 15px box). | Browser-verified all three; suite green. |
